        ...
        public ApprovalResponseModel ProcessApprovalActions(ApprovalRequestModel approvalRequestModel, String token)
        {
            ...
            try
            {
                ...
                if (!response.success)
                {
                    ...
                    else if (responseModel.ErrorMessage.IndexOf("|") > -1)
                    {
                        responseModel.ErrorCode = Convert.ToInt64(responseModel.ErrorMessage.Split("|")[0]);
                        responseModel.ErrorMessage = responseModel.ErrorMessage.Split("|")[1];
                ...
                else
                {
                    ...
                    if (approvalRequestModel.ActionName != null)
                    {
                        if (approvalRequestModel.ActionName.Trim() != "")
                        {
                            ...
                            if (response.result != null && response.result.IndexOf("|") > -1)
                            {
            ...
            catch (Exception ex)
            {
                ...
                if (responseModel.ErrorCode == null)
                {
                    ...
                    insertDNRDLogs(approvalRequestModel.TransactionKey, approvalRequestModel.EventId, JsonConvert.SerializeObject(approvalRequestModel), "Failure", String.Concat("100-Unhandled Exception " + truncatedMessage), startExecutionTime, endExecutionTime, token);
                }
                else
                {
